...
def pred_logistic(x_train, y_train, x_pred, L):
    ...
    popt, pcov = opt.curve_fit(logistic, x_train, y_train, maxfev=5000)
    #print(popt)

    # simple error estimator without intrinsic errors in data points
    sigma_ab = np.sqrt(np.diagonal(pcov))

    y_pred = logistic(x_pred, popt[0], popt[1], popt[2])
    y_pred_upper = logistic(x_pred, popt[0]-1.96*sigma_ab[0], popt[1]-1.96*sigma_ab[1], popt[2]-1.96*sigma_ab[2])
    y_pred_lower = logistic(x_pred, popt[0]+1.96*sigma_ab[0], popt[1]+1.96*sigma_ab[1], popt[2]+1.96*sigma_ab[2])
...
countries = ['Nigeria', 'Italy', 'Germany']
population = [195.9*10**6, 60.5*10**6, 82.8*10**6]
herd_imm_ratio = 0.66
...
for i in range(len(countries)):
    ...
    loc1_str = str(countries[i]).replace(',', ' ') + ","
    loc2_str = "nan,"
    ...
    m_str = str(next_mortality[-1])+ ",nan\n"
