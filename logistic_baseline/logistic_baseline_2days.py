...
for delta in delta_arr:
    ...
    countries = ['Switzerland', 'Italy', 'Germany', 'US', 'France', 'Spain', 'United Kingdom', 'Japan', 'China_Hong Kong', 'Singapore', 'India', 'Iran']
    population = [8.57e6, 60.5e6, 82.8e6, 327.2e6, 66.99e6, 46.66e6, 66.44e6, 126.4e6, 7.50e6, 5.85e6, 1380e6, 83.99e6]
    ...
    file_str = "logistic_baseline_predictions_new/2day_prediction_" + str(today) + ".csv"
    print(file_str)
    ...
    for i in range(len(countries)):
        ...
        if(countries[i]=='China_Hong Kong'):
            confirmed_region = confirmed.loc[confirmed['Country/Region'] == 'China']
            confirmed_region = confirmed_region.loc[confirmed_region['Province/State'] == 'Hong Kong']
        
            deaths_region = deaths.loc[deaths['Country/Region'] == 'China']
            deaths_region = deaths_region.loc[deaths_region['Province/State'] == 'Hong Kong']
        
            recovered_region = recovered.loc[recovered['Country/Region'] == 'China']
            recovered_region = recovered_region.loc[recovered_region['Province/State'] == 'Hong Kong']
